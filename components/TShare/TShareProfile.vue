<template>
  <div>
    ...
    >
      <TCover :width="size" :height="size" :src="profile.photo" />
      <div
      ...
      >
        <div class="flex-grow-0 flex flex-col items-end">
          <div
            class="uppercase py-2 px-8 -mr-8 text-5xl bg-green-500 font-bold"
          >
            Dancer
          </div>
        ...
        <div class="flex-grow-0">
          ...
          <div class="flex items-center mt-4">
            ...
            <div class="text-4xl font-bold">{{ profile.username }}</div>
          </div>
        </div>
      </div>
    </div>
    <TButton class="m-4" @click="download">Download</TButton>
  </div>
</template>
...
export default {
  ...
  methods: {
    async download() {
      const blob = await html2canvas(this.$refs.canvas)
      saveAs(blob.toDataURL(), `${this.profile.username}.png`)
    }
  }
}
</script>
